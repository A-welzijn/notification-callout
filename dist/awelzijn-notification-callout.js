...
;angular.module('awelzijn.notificationcallout').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('templates/template.html',
    ...
    "<strong ng-show=warning.title>{{warning.title}}</strong>{{warning.message}} </div> </li> </ul> <ul class=messages> <li ng-repeat=\"message in info\"> <div class=\"alert callout alert-dismissible\" role=alert> <a class=\"close float-right\" data-dismiss=alert aria-label=Sluit><i class=\"fa fa-times\"></i></a>\n" +
